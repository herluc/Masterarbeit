...
\chapter{Acoustics in the Frequency Domain}
Helmholtzgleichung herleiten und Bestandteile erklären. Freie Parameter beschreiben. Bezug auf Vibroakustik. Erklären, wo Unsicherheiten liegen und warum sich UQ und insbesodnere statFEM hier lohnt.
...
The Helmholtz equation describes the wave equation in a time-independent manner.  

\section{Wave Equation and Helmholtz Equation}
Explain the differences and derive the helmholtz equation. explain that it tepresents the eigenfunctions.
For the Helmholtz equation there holds
\begin{equation}
\nabla^2 p + k^2 p = -Q(x)
\label{eqn:Helmholtz}
\end{equation}
with $p [\SI{}{\pascal}]$ the pressure field, $Q$ the source term and $k$ the wave number. $k$ is defined a\begin{equation}
...
The strong formulation (PDE) is multiplied with a so-called test function $v$ and integrated over the domain $\Omega$:
\begin{equation}
...
\chapter{The Classical Finite Element Method}

\chapter{Probability Theroy and Bayesian Inference}
...
If a kernel function is evaluated at a finite number of points, one gets a covariance matrix. This can be used as tthe covariance of a Gaussian vector from which samples can be drawn. The values of each entry are a function over the input points of the kernel function.
...
\paragraph{stationary kernels}: a function of $x-x'$. Stationary because it doesnt matter what value $x$ and $x_prime$ have as a starting value, the difference is always the same. Example: $1100mm -1000mm = 100 mm, 100100mm-100000mm = 100mm$
...
\paragraph{characteristic length scale}: can be thought of as the number of "upcrossings" in the interval, i.e. the number of crossings of the x axis. can be calculated with eq 4.3. rasmussen. the more upcrossings, the shorter the length scale.
...
\chapter{The Statistical Finite Element Method}
...
\section{Step 1: Posterior of the FEM Forward Model}
...
\subparagraph{find Prior mean and variance}
...
\paragraph{derive marginal likelihood}
\paragraph{estimate Hyperparameters for $C_d$ and scaling factor}
...
\section{Step 2: Estimation of Hyperparameters }
...
\subsection{Minimization of the Negative log-Likelihood}
