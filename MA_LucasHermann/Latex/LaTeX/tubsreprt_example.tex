...
\nomenclature[N]{$\rho$}{density [kg/m**3]}
\nomenclature[N]{$\nabla$}{nabla operator}
...
{python}
a,b = 2,2
mesh = UnitSquareMesh(self.a,self.b) #define mesh
\end{minted}
...
g = rho * omega**2 * U
#variational Problem:	
a = inner(nabla_grad(self.u), nabla_grad(v))*dx - \
 k**2 * inner(self.u,self.v)*dx 
...
The differential equation is treated from a Bayesian viewpoint: all parameters are random variables. For the Poisson equation \eqref{eqn:PoissonEq} these parameters are $\kappa(x)$ and $f(x)$, for the Helmholtz equation \eqref{eqn:Helmholtz} the random parameters could be the wave number $k$ or, as it will be shown in Section \ref{sec:2DFEMprior}, a parameter in the BCs such as the displacement $\bar{U}$. For distributed parameters, such as a source term $f(x)$ dependent on the spatial coordinate, not a single distribution but a GP is applied. Therefore prior to solving the FEM linear system, the parameter $ \mathcal{GP} ( \bar{\bm{f}},\bm{C}_f)$ is sampled. That sample is evaluated for each cell in the FEM mesh which makes it necessary to assemble the system matrix with that in mind. 
$\bar{\bm{f}}$ is set to a prescribed value. $C_f$ is calculated using the prescribed hyperparameters of the chosen kernel function. As an example, a sample from $ \mathcal{GP} ( \bar{\bm{f}},\bm{C}_f)$ can be drawn for a Mat\'ern kernel by calling
\begin{minted}
[
frame=lines,
framesep=2mm,
...
\centering
\caption[Posterior of the 2D Helmholtz equation example for different numbers of sensors and observations for a 0.75x scaled prior as ground truth]{The variance of the posterior for $500Hz$ at all points in the 2D space for different combinations of sensor and observation numbers. Sampled from a $75\%$ scaling. Regions with high and low variance are clearly visible. Although the observations were taken from the scaled sample, the variance quickly gets smaller with more sensors and observations.}
...
\centering
\caption[Comparison of prior and posterior in a 3D view for the 0.75x scaling]{See the \href{https://github.com/herluc/Masterarbeit/blob/master/MA_LucasHermann/Python/Results/2D/75procent_no_d/3DMSE.gif}{Animation} to see the differences better. With observations on a 75\% scaling of the FEM prior and no other introduced model error, the posterior is able to match the data with an MSE of $0.0$. }
...
def doFEM(self):
	...
	class BoundaryX_L(SubDomain):
		self.tol = 1E-14
		def inside(self, x, on_boundary):
			return on_boundary and near(x[0], 0, tol) and \
				(x[1] < sourceBCendCoord)
	...
	self.ds = ds
   #variational Problem
	a = inner(nabla_grad(self.u), nabla_grad(self.v))*dx - self.k**2 * \
