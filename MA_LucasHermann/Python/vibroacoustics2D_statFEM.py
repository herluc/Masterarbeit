...
class solverClass:
	...
	def get_U_mean(self):
		...
		L_imp_mean = (self.v*self.k * self.beta * self.u)*self.ds(1)
		b_imp_mean = assemble(L_imp_mean)

		self.b_mean = b_mean
		#self.bc.apply(b_mean)
		A = self.A
		solve(A,U_mean,b_mean)
		U_imp_mean = spsolve(self.A_comp + 1j*self.b_imp.array(),(b_mean.get_local() ))
		self.U_mean = U_mean
...
######################################
	...
	def get_C_f(self):
		...
		c_f = self.matern_log(self.dof_coordinates, self.dof_coordinates, l=np.log(0.8), sig=np.log(9))
		self.c_f = c_f
	...
	def doFEM(self):
		...
		#	+self.k**2 * (inner(u_r, v_r) - inner(u_i, v_i)) * dx \
		#	+self.k**2 * (inner(u_r, v_i) - inner(u_i, v_r)) * dx

		#L = (v_r*self.g*f0)*ds(sb) + (self.k * imp * u_i + v_i)*ds(1) #WRONG!!! Correct maths before running it! # 1 is the right side. this
		self.beta = 0.5
		L_imp = (self.v*self.k * self.beta * self.u)*ds(1)
		###
		...
		#solve(A, U, b)
		solve(A,U,b)
		self.A_comp = scipy.sparse.csr_matrix(A.array(), dtype = np.cfloat)
		U_imp = spsolve(self.A_comp + 1j*b_imp.array(),(b.get_local() ))
		print("A_c:")
		print(self.A_comp)
		print("b:")
		print(b.get_local())
...
#		plt.grid()
	...
	def doActiveLearning(self,varField):
		...
		var = np.zeros((self.a+1,self.b+1))

		for i in range((self.a+1)*(self.b+1)):
			a,b = int(self.a*X[i]),int(self.b*Y[i])
			var[a,b] = Z[i]
		indices = np.unravel_index(var[1:-1,1:-1].argmax(), var.shape)
		#indices = np.unravel_index(var.argmax(), var.shape)
		newPoint=[0,0]
		newPoint[0] = indices[0]+1
		newPoint[1] = indices[1]+1
		print("DID IT!")
		return newPoint
	...
	#	ax.plot(x,0.7*np.ones(self.a+1),np.array(self.U_mean_cut) - 1.96*np.array(sigL),color="green")
		...
		ax.set_ylim(0,1)
		#ax2.set_ylim(-120,80)
		ax2.set_ylabel("Pressure at $y_c = 0.7$",color='green', loc="top")
		...
		ax3.set_xticks(np.linspace(-20,20,3))
		ax3.set_xlim(-30,60)
	#	ax.set_zlim(-15,15)
		#ax.set_zticks(np.linspace(0,0.15,5))
		fig4.legend(loc="lower right",bbox_to_anchor=(1,1),bbox_transform=ax.transAxes)
...
#for r in [30]:
	...
	for i in range(20):
		...
		i1,i2=rand_ind_x,rand_ind_y
		#i1,i2=1,0
#		if len(solver.postList)>=1:
#			newPoint = solver.doActiveLearning(solver.postList[-1])
		try:
			newPoint = solver.doActiveLearning(solver.postList[-1])
...
figPost.subplots_adjust(bottom=0.1,top=0.9,left=0.1,right=0.8, wspace=0.4,hspace=0.2)
