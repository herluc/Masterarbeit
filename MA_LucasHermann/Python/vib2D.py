...
class solverClass:
	def __init__(self):
		...
		self.a,self.b = 24,24
		self.mesh = UnitSquareMesh(MPI.COMM_WORLD,self.a,self.b) #define mesh
		...
		class MyExpression1:
			...
			def eval(self, x):
				...
				if x[0] <(0.0+1e-8):
					...
					return 0.0 #only the boundary is non-zero. evry other point is zero.
			def value_shape(self):
				return ()
...
######################################
	...
	def doFEM(self):
		...
		boundary_markers = dolfinx.MeshFunction("size_t", self.mesh, self.mesh.topology().dim()-1	, 0)
		...
		L = (self.v*self.g*self.f_fun)*ds(sb)
		self.f_fun.interpolate(self.f0.eval)
